...
public class CharacterController2D : MonoBehaviour {
	...
	public int maxSalve = 20;
	public int currentSalve;
	...
	void Start(){
		currentSalve = maxSalve;
		rainBar.SetMaxSalve(maxSalve);
	}
	...
	void Update()
	{
		...
		else if(canDoubleJump && Input.GetButtonDown("Jump")) //If can double jump AND jump button pressed, then allow the player to double jump
		{
			DoJump();
			canDoubleJump = false;
		}
		...
		//Test the rain bar by pressing L
	...
	public void ApplyDamage (int damage) {
		if (playerCanMove) {
			currentSalve -= damage;
			rainBar.SetSalve(currentSalve);

			if(currentSalve <= 0 && playerHealth >=1){
				playerHealth -= damage;
			}

			if (currentSalve <= 0 && playerHealth <= 0) { // player is now dead, so start dying
				PlaySound(deathSFX);
	...
	public void CollectSalve(int amount){
		PlaySound(salveSFX);

		currentSalve += amount;  //add the points throught the character controller, if it is available
		rainBar.SetSalve(currentSalve);

		//if the current salve is full, add a 1up+
	}
